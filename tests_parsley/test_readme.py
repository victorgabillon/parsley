...
def test_precedence() -> None:
    ...
    # Parse arguments with extra_args
    config = parser.parse_arguments_with_command_line_args(
        config_file_path="tests_parsley/yaml_files/config2.yaml",
...
def test_discriminator() -> None:
    ...
    # Parse arguments with extra_args
    config = parser.parse_arguments_with_command_line_args(
        config_file_path="tests_parsley/yaml_files/discriminator.yaml"
    )
...
def test_discriminator2() -> None:
    ...
    # Parse arguments with extra_args
    config = parser.parse_arguments_with_command_line_args(
        config_file_path="tests_parsley/yaml_files/discriminator2.yaml"
    )
...
class ModelPreset(str, Enum):
    ...
    def get_yaml_file_path(self) -> str:
        return f"tests_parsley/yaml_files/preset_{self.value}.yaml"
...
def test_yaml_path_method() -> None:
    ...
    # Parse arguments with extra_args
    config = parser.parse_arguments(
        config_file_path="tests_parsley/yaml_files/config_model.yaml"
    )
